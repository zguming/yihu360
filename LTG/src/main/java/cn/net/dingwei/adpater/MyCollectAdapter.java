...
public class MyCollectAdapter<T> extends TreeListViewAdapter<T>{
	...
	public MyCollectAdapter(ListView mTree, Activity context, List<T> datas,int defaultExpandLevel)
		...
		progress_width = Screen_width - DensityUtil.DipToPixels(context, 140);//减去其他控件的宽度 剩下进度条的宽度

		drawable1 = MyFlg.setViewRaduis(color_101, color_101, 1, 0);
		drawable2 = MyFlg.setViewRaduis(color_102, color_102, 1, 0);
		drawable3 = MyFlg.setViewRaduis(color_103, color_103, 1, 0);
	}
	...
	public View getConvertView(final Node node , int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView == null)
		{
			convertView = mInflater.inflate(R.layout.item_duoji_listview_collect, parent, false);
		...
		viewHolder.image_lianxi.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(MyApplication.Getget_user_baseinfo(mContext).getCtgg_need_vip()==1&&(MyApplication.getuserInfoBean(mContext).getMember_status()!=1 && MyApplication.getuserInfoBean(mContext).getMember_status()!=2)){
					...
					params.add("exercises_type","sclx");
